...
export default function RealizationTargetLineChart(): JSX.Element {
    ...
    dashboard.data?.grafik_target_realisasi?.forEach((target_realisasi_item) => {
        months.forEach((month) => {
            if (target_realisasi_item?.month === month.value) {
                labels.push(month.name.slice(0, 3))
            }
        })

        datasets.push(target_realisasi_item?.realisasii)
